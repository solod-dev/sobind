...
// macroValue returns the Go constant expression for an object-like macro, plus
// a note for the reader when the macro names a value sobind cannot express.
// Both are empty for a macro that is not a constant at all (#define API extern).
func macroValue(m *cc.Macro, macros map[string]*cc.Macro) (value, note string) {
	switch classifyMacro(m, macros, map[string]bool{}) {
	case macroInt:
	...
	case macroFloat:
		...
		return "", "float expression, define the constant manually"
	case macroString:
		toks := resolveMacro(m, macros)
		if len(toks) != 1 {
			return "", ""
		}
		return toks[0].SrcStr(), ""
	}
	return "", ""
}
...
func classifyMacro(m *cc.Macro, macros map[string]*cc.Macro, seen map[string]bool) macroKind {
	...
	hasFloat, hasString := false, false
